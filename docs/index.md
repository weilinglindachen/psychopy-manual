...
這份手冊記錄本研究使用 PsychoPy Builder 建置實驗的完整流程。整體工作分為 **四個階段**，每個階段都會先完成一個可獨立測試的版本，再進入下一階段。
...
| 第三階段 | 任務 9–11 | 擴充成完整 48 trials，並檢查四種條件是否平衡 | ✅ 已完成 |
| 第四階段 | 任務 12–18 | 完成受試者實際看到的完整實驗（說明、練習、休息、結束畫面） | ✅ 已完成 |
...
**2026/08/16**：完成第三、第四階段

1. 建立完整 48-trial 條件表，並確認四種條件（C+R、C+I、V+R、V+I）平衡
2. 設定固定的 pseudorandom trial 呈現順序，完成 48-trial 正式測試與 CSV 驗證
